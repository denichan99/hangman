import pygame
import sys
from mechanics import initialize_game, get_display_word, is_game_over, process_guess, get_random_word
...
# Маппинг символов для русской раскладки (pygame не поддерживает русскую раскладку)
RUSSIAN_KEYMAP = {
...
def draw_hangman(attempts_left): #отображение виселицы
    if attempts_left < 7:
...
def draw_game_state(game_state, message):
    ...
    screen.blit(word_surface, (500, 300))
...
def handle_user_input(game_state, message): #Обработка ввода пользователя
...
def game_loop(word): #Основной игровой цикл
    ...
    if all(letter in game_state["guessed_letters"] for letter in game_state["word"]):
        ...
        screen.blit(win_surface, (WIDTH // 2 - win_surface.get_width() // 2, 430))
    else:
        lose_text = "Вы проиграли! Загаданное слово: " + game_state["word"]
        lose_surface = font.render(lose_text, True, RED)
        screen.blit(lose_surface, (WIDTH // 2 - lose_surface.get_width() // 2, 430))
